...
// 对于 if, while, for 在花括号里定义的变量，除了花括号就被释放了，
    int a = 1;
    for (int a = 2; a == 2; ++a)
    {
...
- 修饰普通函数：表明函数的作用范围，仅在定义该函数的文件内才能使用。在多人开发项目时，为了防止与他人命名的函数重名，可以将函数定位为 static。
...
- map 的优点在于元素可以自动按照键值排序，而 hash map的优点在于它的各项操作的平均时间复杂度接近常数
...
- shared_ptr被称为共享指针，用于管理多个智能指针共同拥有的动态分配对象， 
-  唯一拥有指定的对象，相比普通指针，拥有RAII的特性使得程序出现异常时，动态资源可以得到释放。 
